...
export default async function main({db, covers}){
    ...
    const bar = progress(`resizing images [:bar] :rate/tpf :percent :etas`, db.length);
    let counter = 0;
    for (const post of db){
      ...
      for(const {prefix, width, height} of covers){
        ...
        if(!exists) await resize({width, height, src, dest});
      }
      // await zzz();
      bar.tick()
      counter++;
...
async function resize({width, height, src, dest,}){
  return sharp(src).resize(width, height).jpeg().toFile(dest); // https://sharp.pixelplumbing.com/
}
