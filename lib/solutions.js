...
export default async function solutions(context){
  log.info('Solutions Phase')
...
async function missing(file){ // .dest.target
  ...
  if(exists && file.directory){ // dig deeper
    const sourceFiles = (await fs.readdir( file.src, { withFileTypes: true })).filter(dirent => dirent.isDirectory());
    const destinationFiles = (await fs.readdir( file.dest.target, { withFileTypes: true })).filter(dirent => dirent.isDirectory());
