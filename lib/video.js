...
export default async function video({db, configuration:{video} }){
  ...
  const bar = progress(`generating video [:bar] :rate/tps :percent :etas`, selected.length);
...
async function makeVideo(record, video){
  ...
  const image = path.join(record.file.files.src, record.attr.image);
  try {
