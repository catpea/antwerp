...
export default async function titles({db, configuration:{pp, dest, theme}, site}){
  const filename = 'tiles.html';
  const data = Object.assign({filename},{posts:db}, site);
  const html = await template({ theme, file: path.join('tiles','index.ejs'), data, });
