...
export default async function recon({db, configuration:{pp, dest, theme}, site}, options){
  const bar = progress(`loading images+links`, `[:bar] :rate/tpf :percent :etas`, db.length, options.progress);
